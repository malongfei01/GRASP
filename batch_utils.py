...
from torch_geometric.loader import NeighborSampler, ClusterData, ClusterLoader, GraphSAINTNodeSampler,GraphSAINTEdgeSampler,GraphSAINTRandomWalkSampler,RandomNodeLoader
...
def dataset_add_masking(dataset, idx):
    mask = torch.zeros(dataset.num_nodes, dtype=torch.bool)
    mask[idx] = True
    dataset.node_ids = torch.arange(dataset.num_nodes)
    dataset.mask = mask
    return dataset
...
def make_loader(args, dataset, idx, mini_batch=True, save_dir=None, device=torch.device('cpu'), test=False):
    if not mini_batch:
        dataset = dataset_add_masking(dataset, idx)
        # full batch test right now
        loader = RandomNodeLoader(dataset, num_parts=1, shuffle=True, num_workers=0)
        return loader
        
    if args.train_batch == 'cluster':
        dataset = dataset_add_masking(dataset, idx)
        cluster_data = ClusterData(dataset, num_parts=args.num_parts, save_dir=save_dir)
        ...
        dataset = dataset_add_masking(dataset, idx)
        if not test:
            loader = GraphSAINTNodeSampler(dataset, batch_size=args.batch_size, shuffle=True, num_workers=0, num_steps=args.saint_num_steps)
        else:
            loader = RandomNodeLoader(dataset, num_parts=args.test_num_parts, shuffle=True, num_workers=0)
            
    elif args.train_batch == 'graphsaint-edge':
        dataset = dataset_add_masking(dataset, idx)
        if not test:
            loader = GraphSAINTEdgeSampler(dataset, batch_size=args.batch_size, shuffle=True, num_workers=0, num_steps=args.saint_num_steps)
        else:
            loader = RandomNodeLoader(dataset, num_parts=args.test_num_parts, shuffle=True, num_workers=0)
        ...
        dataset = dataset_add_masking(dataset, idx)
        if not test:
            loader = GraphSAINTRandomWalkSampler(dataset, batch_size=args.batch_size, walk_length=args.num_layers, shuffle=True, num_workers=3, num_steps=args.saint_num_steps)
        else:
            loader = RandomNodeLoader(dataset, num_parts=args.test_num_parts, shuffle=True, num_workers=0)
            
    elif args.train_batch == 'random':
        dataset = dataset_add_masking(dataset, idx)
        loader = RandomNodeLoader(dataset, num_parts=args.num_parts, shuffle=True, num_workers=0)
