...
class GAT(nn.Module):
    ...
    def forward(self, data, adjs=None, x_batch=None, return_feature=False):
...
class MultiLP(nn.Module):
    ...
    def forward(self, data, train_idx):
        ...
        if data.y.shape[1] == 1:
            # make one hot
            y[train_idx] = F.one_hot(data.y[train_idx], self.out_channels).squeeze(1).to(y)
        elif self.mult_bin:
            y = torch.zeros((n, 2*self.out_channels)).to(adj_t.device())
            for task in range(data.y.shape[1]):
                y[train_idx, 2*task:2*task+2] = F.one_hot(data.y[train_idx, task], 2).to(y)
        else:
            y[train_idx] = data.y[train_idx].to(y.dtype)
        result = y.clone()
...
class H2GCN(nn.Module):
    ...
    def forward(self, data, return_feature=False):
        ...
        adj_t = self.adj_t.to_device(x.device)
        adj_t2 = self.adj_t2.to_device(x.device)
